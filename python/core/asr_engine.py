...
from core.config import MODEL_DIR, HOTWORDS_PATH, HOTWORD_SCORE, SAMPLE_RATE
...
def create_recognizer():
    ...
    print(f"[INIT] Loading Zipformer model from {MODEL_DIR}...", flush=True)
    ...
    return sherpa_onnx.OnlineRecognizer.from_transducer(
        ...
        decoding_method="modified_beam_search", # Required to apply hotwords biasing
        max_active_paths=4,                     # Limits search tree size to reduce CPU load
        
        # Hotword biasing configuration
        hotwords_file=HOTWORDS_PATH,
        hotwords_score=HOTWORD_SCORE,
        
        # Audio stream and runtime parameters
        num_threads=2,
        sample_rate=SAMPLE_RATE,
        feature_dim=80,
        
        # Speech silence boundary (endpointing) rules
        enable_endpoint_detection=True,
        rule1_min_trailing_silence=2.4,  # Silence duration to split long sentences
        rule2_min_trailing_silence=1.2,  # Silence duration to split short utterances
